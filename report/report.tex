...
The second part is where we present the experminental results.
...
As $ \theta $ is not necessarily an angular representation of a cut, we need to develop a method that associates a cut $ x \in \left\{ -1,1 \right\}  $ to any $ \theta \in
\mathbb{R} ^{ n }  $.
...
\item The cut generated by assignment \ref{eq:assigncut} can also be generated by a Goemans-Williamson cut. This is achieved by simply slightly turning the diving line in the
...
%This tweak makes sense as diametrically opposed points have the highest contribution to the weight
\improvement{Type out why this is good}
%We will see that reason for this tweak is that in algorithm \ref{alg:ProcedureCUT} the angle is updated such that there is at least one $ i \in \left[ 1:n \right]  $ such that
\end{enumerate}
\end{rem} 
...
\begin{rem}
Algorithm 2 finds all Goemans-Williamson-type cuts. 
\end{rem} 
...
We review the probabilities that two points $ v_i $ and $ v_j $ are separated by a Goemans-Williamson-type cut, for any $ i,j \in \left[ 1:n \right]  $. 
...
\end{cases}
\end{align*} 
Clearly, if two points are separated by a Goemans-Williamson cut they are also separated by a Goemans-Williamson-type cut. 
Thus for an angle $ \beta $ the points $ v_i $ and $ v_j $ are separated by a Goemans-Williamson-type cut but not a Goemans-Williamson cut if and only if $ \beta = \theta_i - \frac{ \pi  }{ 2 }  $ or $ \beta =
\theta_j -
\frac{ \pi  }{ 2 }  $.
Since the set of separating angles only changes on a nullset the probabilities remain unchagned.
...
Since algorithm \ref{alg:ProcedureCUT} finds all Goemans-Williamson-type cuts it finds a Goemans-Williamson-type cut of value greater or equal to $ \alpha \psi ( \theta) $.
...
Recall, that in Goemans-Williamson algorithm, the guarantee comes from (approximately) solving the \SDP \ref{def:relaxedsdp}.
...
\end{algorithmic}
\end{algorithm}
\todo[inline]{reference of algorithms does not work! fix in whole file!}
...
However as the termination criterion in \textcolor{red}{IMPROVEDCUT} relies on the number of unsuccessful attempts we can not predict the runtime.
Since there are elements of randomness, namely in the choice of the seed and the random pertubation there can always be exceptions to the rule.
...
The parameter N, which determins the number of consecutive tries for improving a cut and the parameter M, which controls the number of starting points.
We are going to compare 21 \textcolor{red}{solvers}, which are the combinations of $ M \in \left\{ 1, 5, 10, 15, 20, 30, 40, 50 \right\}  $ and $ N \in \left\{ 5, 10 , 15 \right\}  $.
...
The \textcolor{red}{solvers} are run on the instances from \MallachLibrary.
...
The number of vertices ranges from 21 to 3000 with a median of $ 101 $ and a mean of 394.69 vertices.
\textcolor{red}{There are sparse and dense graphs, (though most graphs are sparse?)}
For 
...
Regarding the minimum the same phenomen occurs.
For the two instances gka10b and gka9b the minimal ratios are 0.000000 and 0.277372 respectively.
For the other instance the minmal ratio is greater than 0.97 exceeding \GWconst.
...
The standard deviation for gka9b and gka10b is with 0.331967 and 0.154788 respectively orders of magnitude larger than for the rest which have stadard deviation less or equal
to 0.004201.
...
\todo[inline]{Add plot}
First of all we notice that the quality of the returned cut strongly varies and that it keeps oscilating even for big parameters.
...
Performing a similar analysis as above

\subsection{Results} 
The experiments gather further data that the \BH is highly effective in returning high quality approximations.
Out of the 332 test instances with known optimum value, the instances for which the heuristic \textcolor{red}{returns a cut with ratio less than 0.878 (the
...
The data clearly suggests that using the \BH to generate high quality approximate solutions is highly effective.
