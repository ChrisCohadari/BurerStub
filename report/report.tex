...
\improvement{Improve this sentence}
We will conduct experiments on the library \cite{MallachLibrary} asserting the effectiveness in section \ref{sec:expermints}. %to investigate this trade-off. This yields data demonstrating the effectiveness.
...
the \BH Mxx.Nyy.
In the same manner, the bottom plot tracks the average number of generated cuts for each solver over all instances with known optimal value.
...
cuts.
\unsure{Maybe stop here}
\textcolor{red}{Lastly we can see that increasing only the parameter N increases the average number of generated cuts by a fairly consistent constant.}
...
\caption{Overview of the returned ratios and number of generated cuts for each solver on the instance gka9b.}
\label{plot:gka9b} 
\end{figure} 
...
\label{plot:gka10b} 
\end{figure} 
In figure \ref{plot:gka10b}  we notice that the quality of the returned cut strongly varies and that it keeps oscillating even for big parameters.
...
In particular the heuristic returns high quality cuts for instances with mixed sign weights, these are \textcolor{red}{how many are they?}
...
\todo[inline]{Decide for Goemans-Williamson or Goemans Williamson}
